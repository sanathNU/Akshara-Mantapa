...
use akshara_mantapa::{
    engine::GraphemeAlphabet, LibraryOfBabel, Location, BOOKS_PER_SHELF, CLUSTERS_PER_PAGE,
    PAGES_PER_BOOK, SHELVES_PER_WALL, WALLS_PER_ROOM,
};
...
#[derive(Clone)]
struct AppState {
    kannada: Arc<LibraryOfBabel<GraphemeAlphabet>>,
    telugu: Arc<LibraryOfBabel<GraphemeAlphabet>>,
    tamil: Arc<LibraryOfBabel<GraphemeAlphabet>>,
}
...
fn library_for_script(
    state: &AppState,
    script: Option<&str>,
) -> Arc<LibraryOfBabel<GraphemeAlphabet>> {
    match script.unwrap_or("kannada").to_lowercase().as_str() {
        "telugu" | "telegu" | "telu" => Arc::clone(&state.telugu),
        "tamil" | "taml" | "tam" => Arc::clone(&state.tamil),
        _ => Arc::clone(&state.kannada),
    }
}
...
async fn health_check(State(state): State<AppState>) -> impl IntoResponse {
    Json(serde_json::json!({
        "status": "ok",
        "default_script": "kannada",
        "supported_scripts": [
            SupportedScript {
                code: "kannada",
                name: state.kannada.script_name().to_string(),
                script_code: state.kannada.script_code().to_string(),
                is_default: true,
            },
            SupportedScript {
                code: "telugu",
                name: state.telugu.script_name().to_string(),
                script_code: state.telugu.script_code().to_string(),
                is_default: false,
            },
            SupportedScript {
                code: "tamil",
                name: state.tamil.script_name().to_string(),
                script_code: state.tamil.script_code().to_string(),
                is_default: false,
            },
        ],
    }))
...
async fn get_library_info(
    ...
    let library = library_for_script(&state, query.script.as_deref());
    ...
    Json(LibraryInfo {
        name: library.script_name().to_string(),
        script_code: library.script_code().to_string(),
        alphabet_size,
...
async fn get_random_page(
    State(state): State<AppState>,
    Query(query): Query<ScriptQuery>,
) -> impl IntoResponse {
    let library = library_for_script(&state, query.script.as_deref());
    let page = library.random_page();

    Json(PageResponse {
        raw_address: page.location.raw_hex.clone(),
        hierarchical: make_hierarchical_display(&library, &page.location),
...
async fn get_page_by_address(
    ...
    let library = library_for_script(&state, query.script.as_deref());
    let location = match library.parse_address(&query.address) {
        Some(loc) => loc,
        None => {
            return (
    ...
    let page = library.generate_page(&location);
...
async fn get_next_page(
    ...
    let page = library.next_page(&location);
...
async fn get_previous_page(
    ...
    match library.previous_page(&location) {
        Some(page) => Json(PageResponse {
            raw_address: page.location.raw_hex.clone(),
            hierarchical: make_hierarchical_display(&library, &page.location),
            content: page.content,
            formatted_content: page.formatted_content,
...
async fn search_text(
    ...
    match library.search_at_random_position(&query.q) {
        Some(result) => {
            ...
            (
                ...
                Json(SearchResponse {
                    ...
                    location: Some(LocationResponse {
                        ...
                        hierarchical: make_hierarchical_display(&library, &result.location),
                    }),
...
async fn verify_text(
    ...
    let library = library_for_script(&state, params.script.as_deref());
    let location = match library.parse_address(&params.address) {
        Some(loc) => loc,
        None => {
            return (
                StatusCode::BAD_REQUEST,
...
async fn main() {
    ...
    let state = AppState {
        kannada: Arc::new(LibraryOfBabel::kannada()),
        telugu: Arc::new(LibraryOfBabel::telugu()),
        tamil: Arc::new(LibraryOfBabel::tamil()),
    };
    ...
    println!("║  A Library of Babel for Kannada, Telugu, and Tamil");
    println!("╠══════════════════════════════════════════════════════════════╣");
    ...
    println!("║    GET /api/search            Find text in random content    ║");
